<?php
	class EDC_PAGE_TARIFFS{
		...
		protected function importTariffs(){
			...
			if(!file_exists(EDC_PLUGIN_PATH.'/inc/tariffs_importer.class.php')) return EDCAdmin::inst()->ajaxResult('error',__('Plugin files are corrupt. You can not use importer.','edc'));
			require_once EDC_PLUGIN_PATH.'/inc/tariffs_importer.class.php';
		...
		private function getTariffFormTpl($tid=''){
			...
			if(is_numeric($tid)){
				$data['page_title']=__('Edit tariff - #'.$tid,'edc');
