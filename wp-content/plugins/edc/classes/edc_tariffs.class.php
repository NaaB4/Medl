...
class EDC_TARIFFS extends EDCH{
	...
	static function getList($params=array()){		
		...
		if($limit!=''){
			$query_count="SELECT COUNT(*) as `cnt` FROM `".self::table('tariffs')."` t WHERE 1 ".(sizeOf($cond)==0 ? '' : ' and '.implode(' and ',$cond))."";
	...
	static function getOptions($type){
		...
		$options[]=array(
			'key'=>'edc_bookable',
			'type'=>'checkbox',
			'label'=>__('Tariff bookable online','edc'),
			'default'=>true,
		);
		$options[]=array(
			'key'=>'edc_no_book_link',
			'type'=>'text',
			'name'=>__('Tariff no book link','edc'),
			'description'=>__('If you will leave this option empty, then will be used global setting "Request form settings"')
		);
		if($type==1){
			$options[]=array(
	...
	static function getPrices($tid='',$data=null){
		...
		return self::getPrice($data->price_per_period,$data->price_per_kwh,$args);
	}
