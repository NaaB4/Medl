	...
	<?php else : ?>
		...
		<?php foreach($data['orders'] as $order) : ?>
			<div class="item">
				...
				<?php if(isset($order->options['self_mail_sent']) || isset($order->options['customer_mail_sent'])) : ?>
				<div class="infos">
					<?php if(isset($order->options['self_mail_sent'])) :?>
						<span class="<?=(EDCH::is($order->options['self_mail_sent']) ? 'green' : 'red')?>"><?=__('Mail an medl','medl')?>: <span class="value"><?=(EDCH::is($order->options['self_mail_sent']) ? __('Ja','medl') : __('Nein','medl'))?></span></span>
					<?php endif; ?>
					<?php if(isset($order->options['customer_mail_sent'])) :?>
						<span class="<?=(EDCH::is($order->options['customer_mail_sent']) ? 'green' : 'red')?>"><?=__('Mail an Kunden','medl')?>: <span class="value"><?=(EDCH::is($order->options['customer_mail_sent']) ? __('Ja','medl') : __('Nein','medl'))?></span></span>
					<?php endif; ?>
				</div>
				<?php endif; ?>
				<div class="details">
					...
					<?php if(file_exists($order->options['order_csv_file'])) : ?>
						<a type="button" class="btn" href="<?=admin_url('admin.php?page=edc_orders&csv='.$order->id)?>" target="_blank"><?=__('CSV','edc')?></a>
					<?php endif; ?>
				</div>
			</div>
		<?php endforeach; ?>
