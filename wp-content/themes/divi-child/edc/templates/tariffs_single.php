<?php
	...
	$dop_class='';
	if($data['tariff']->options['landing_page_uniq']=='heimkehrer'){
		$dop_class.=' heimkehrer';
	}
?>	
<div class="edc item">
    <div class="single_tariff theme_<?=EDCH::proceedType($data['tariff']->type,true)?><?=$dop_class?>" id="edc_tariff_<?=$data['tariff']->id?>" data-tarif="<?=$data['tariff']->id?>">
		<div class="main_info">
		...
		<?php if($data['tariff']->type!='combi') : ?>
			...
			<div class="prices">
				<ul>
					...
					<li>
						Grundpreis (brutto) pro <b>Jahr</b>:
						<?=EDCH::displayPrice($data['tariff']->prices['base_per_year'],'ppy','Euro')?> 
					</li>
					<?php else : ?>
					<li>
						Grundpreis (brutto) pro <b>Monat</b>:
		...
		<?php if($total_goodies>0) : ?>
			<div class="goodies_list">
				<div class="heading"><?=__('Bonus wählen','medl')?></div>
			<?php foreach($goodies as $goodie) : ?>
				<div class="checkbox field to_top">
					<input type="radio" id="goodie_<?=$goodie->id?>_<?=$data['tariff']->id?>" name="goodies" value="<?=$goodie->id?>" onchange="edc.recalculatePrice(this);">
