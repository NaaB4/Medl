...
class EDC_ORDERS extends EDCH{
	...
	static function add($data,$options=array()){
		...
		$arr['pdf_name']=isset($data['pdf_name']) ? $data['pdf_name'] : 'Bestellung #'.$new_oid;
		//var_dump($arr);
	...
	static function createCSV($oid,$data){
		...
		$fields=[
			...
			'Lieferbeginn'=>'edc_electriс_date',
			'Vorversorger Strom'=>($edc->steps_data['first']['type']=='electricity' ? 'edc_previous' : ''),
			'Vorversorger Gas'=>($edc->steps_data['first']['type']=='gas' ? 'edc_previous' : ''),
		...
		$fname=wp_generate_password(mt_rand(30,50),false).'.csv';
