...
class EDCH{
	...
	static $types=array('any'=>0,'gas'=>1,'electricity'=>2,'combi'=>3);
	...
	static function getResultPageURL($tid=''){ return EDC_HELPER::getResultPageURL($tid); }
	...
	static function dateToDatabase($date=''){ return EDC_HELPER::dateToDatabase($date); }
	...
	static function getIds($ids,$max=0,$delim=array('::',',','|',';',"\n")){
		if(!is_array($ids)){
			$replace=array();
			if(!is_array($delim)) $delim=array($delim);
			foreach($delim as $v) $replace[]='|';
			$ids=str_replace($delim,$replace,$ids);
			$ids=explode('|',$ids);
		}
		self::trimArray($ids);
		$i=0;
	...
	static function getIdsForCond($ids=''){
		$ids=self::getIds($ids);
		if(sizeOf($ids)==0) return '';
		$res='';
		foreach($ids as $id) $res.=','.self::DB()->escape($id);
		return substr($res,1);
	}
	static function table($name=''){
		if($name=='') return false;
		$tables=array('options','tariffs','tariff_postcodes','tariff_options','orders','order_options','tariffs_options','postcodes','tariff_postcodes_exclude','goodies');
